	...
	if(($_FILES["file"]["size"] < 40000000) && in_array($extension, $allowedExts))
	{
		...
		else //if there is not an error
		{
			...
			if(($handle = fopen($_FILES["file"]["name"], "r"))!== FALSE)
			{
				...
				//form
				echo "<form action='process_map_customer_type.php' method='post'>";
				...
				echo "<li><label for='mapping_Old_Type'>Old Customer Type:</label><br><select name='mapping_Old_Type'>"; for ($c=0; $c < sizeof($old_labels_array); $c++) { echo "<option value='" . $c . "'>" . $old_labels_array[$c] . "</option>"; } echo "</select></li><br>";
				echo "<li><label for='mapping_New_Type'>New Customer Type:</label><br><select name='mapping_New_Type'>"; for ($c=0; $c < sizeof($old_labels_array); $c++) { echo "<option value='" . $c . "'>" . $old_labels_array[$c] . "</option>"; } echo "</select></li><br>";
